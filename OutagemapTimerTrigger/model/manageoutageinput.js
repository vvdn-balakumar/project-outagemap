...
function processOutageDetails(context, callback) {
    ...
    async.parallel({
        ...
        if (err) {
            ...
            logger.log("In processOutageDetails", new Date());
...
function circuitLevelGroupingCalc(objResults, callback) {
    ...
    if (arrTransformerKeys.length == 0) {
...
function getAlarmAndEvents(callback) {
    ...
    objdaoimpl.getCursorFromCollectionSorted("DELTA_AlarmsAndEvents", ['DBTimestamp'], [{ $gt: objStartDate }], { "DBTimestamp": 1 }, {}, function (err, arrAlarmAndEvents) {
...
function getAllSummaryMapRelatedDetailsTransformer(callback) {
    ...
    objStartDate.setUTCMinutes(0);
    objStartDate.setUTCSeconds(0);
    objdaoimpl.getCursorFromCollection("DELTA_Transaction_Data", ['DBTimestamp'], [{ $gt: objStartDate }], function (err, arrTransformerTransData) {
